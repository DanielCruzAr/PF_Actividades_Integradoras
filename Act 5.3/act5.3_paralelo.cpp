...
class DFA {
	...
	public:
		...
		thread myThread(vector<string>&, int, int);
		void proccesEntry(vector<string>&, int, int);
...
thread DFA::myThread(vector<string> &v, int start, int end){  
	return thread([&] { proccesEntry (v, start, end) ; }); 
} 
...
void DFA::proccesEntry(vector<string> &v, int start, int end){
	char c;
	string s;
...
vector<string> lexerAritmetico(string archivo){
	string line;
	vector<string> v;
...
int main(int argc, char* argv[]) {
	...
	vector<string> vfile = lexerAritmetico(file);
	...
	for (int i = 0; i < THREADS; i++){
		int start = i*block;
		int end = (i == vfile.size() - 1)? vfile.size() : (i + 1) * block;
		starts.push_back(start);
		ends.push_back(end);
	}
	
	thread t1 = dfa.myThread(vfile, starts[0], ends[0]);
	thread t2 = dfa.myThread(vfile, starts[1], ends[2]);
	thread t3 = dfa.myThread(vfile, starts[3], ends[3]);
	thread t4 = dfa.myThread(vfile, starts[4], ends[5]);
